...
int main()
{
    ...
    // -------------------------
    //unsigned int programID = LoadShaders("3.1.blending.vs", "3.1.blending.fs");
    unsigned int programID = LoadShaders("src/4.advanced_opengl/3.1.blending_discard/3.1.blending.vs", "src/4.advanced_opengl/3.1.blending_discard/3.1.blending.fs");
    ...
    // --------------------------------
       ...
       // I tried to use cvector to replace vector, but it was useless.
    vec3 vegetation[5] = {
        {-1.5f, 0.0f, -0.48f},
        {1.5f, 0.0f, 0.51f},
        {0.0f, 0.0f, 0.7f},
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        {
            glm_mat4_identity(model); // Reset the model Matrix
            glm_translate(model, vegetation[i]);
            setMat4(programID, "model", model);
            glDrawArrays(GL_TRIANGLES, 0, 6);
        }
